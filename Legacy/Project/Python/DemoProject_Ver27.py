# change 1 - the way the "unknown_files" counter is placed now means every unknown extension gets counted, 
# even if the file is already sitting in Others/Unknown and doesn’t actually need to be moved. That inflates the stats a little
...
# This increments the counter before you check:


# if source_path != destination_path and os.path.exists(source_path):
#     safe_move(source_path, destination_path)

# So even files that are already in the right place get counted as “unknown.”

# Fix?
# Move the increment inside the actual move condition, so it only happens when the file is truly processed:
# inside:
# if source_path != destination_path and os.path.exists(source_path):
