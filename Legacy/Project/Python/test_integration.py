...
Runs the full Stash pipeline against a disposable temporary directory and
verifies:
  - Correct file moves
...
def run_integration_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="stash_test_") as tmp:
        ...
        logger = StashLogger(str(log_dir), Path(tmp))
        stats = StatsTracker()
        ...
        logger2 = StashLogger(str(log_dir), Path(tmp))
        stats2 = StatsTracker()
        start2 = time.time()
        ...
        check(
            ...
            "Documents" in skip_names and ".system" in skip_names and "Legacy" in skip_names,
        )
