...
# 1st. Use of "files_moved += 1" inside the duplicate file renaming logic inside the "def safe_move()" function.
# Since in the previous versions, the stats were recorded as:
# files processed, files moved, duplicates renamed, unknown files found, all in total (per run/scanning)
# But for the duplicates file, when they are encountered if the file with same name is already present in the destination directory,
# then, the file to be moved is renamed before moving and then it is moved.

# previously, the counter "documents_renamed" only checked and incremented for the files (or when) renamed.
# It did not account for whether the file was actually moved, even if the file was moved after renaming.
# So, to eliminate the gap, incremented the "files_moved counter" inside th logic, where file was renamed before moving.

# 2nd. omitted the use of "files_processed" variable in the unknown files
# Since, when the program runs, it checks for extensions (file type).
# if unknown extension, move to "others" folder,
# files processed = 1

# then,
# safe_move function. Note: above thing (moving of unknown files) happens before the execution of safe_move function.
# inside safe_move function - files processed = 1

# Total files processed = 2
# but actually, only one file was processed. The "unknown" file.

# 3rd. With the previous versions, after introducing the statistics feature, an issue arised.
# The issue was, there is a separate folder for application generated files, by the name ".system". Inside it resides, "logs" folder and "stats" folder.
# The working was, that after the generation of "Statistics.json" file, it will automatically move to ".system/stats" folder.
# But rather, it stayed as "Statistics.json" filein the project folder (or current working directory).

# The Issue was arised by these lines:

# the stats folder creation inside the ".system" folder and then, joining the json file "Statics.json" with the ".system/stats" location
# That is, now the file will go to the above folder ".system/stats" after its creation
...
# but when writing the "Statistics.json" file using python's file methods:

# with open("Statistics.json", 'a') as f:
#    json.dump(stats, f, indent=2)

# here the stats dictionary was being used for all those parameters. But, the file was being saved as "Statistics.json" but in the root/curren working directory.
# instead of being saved inside ".system.stats"
# because the stats folder was created and the "Statistisc.json" file was already linked with it.
# So, to remove the logic gap, used "stats_file" instead of "Statistics.json" since, 
# "stats_file" variable contained the actual logic of moving the "statistics.json" file inside ".system/stats".
...
def safe_move(source_path, destination_path):
    ...
    # Treating and using the stats counter variables as global inside this function
    global files_processed, files_moved, duplicates_renamed, unknown_files, stats
    ...
    if os.path.exists(destination_path):
        ...
        while True:
            ...
            if not os.path.exists(new_name):
                ...
                # managing 'DUPLICATE FILES' stats
                duplicates_renamed += 1   # increment AFTER successful rename

                files_moved += 1 # increment after successfull moving of duplicate file after renaming.

                files_processed += 1 # for total files processed, whether moved or not.
        ...
        files_moved += 1   # increment AFTER successful move
...
for current_path, foldernames, filenames in os.walk(project_folder):
    ...
    for fl in filenames:
        ...
        if extension in all_extensions:
            ...
            # Managing the total number of 'UNKNOWN FILES' moved
            unknown_files += 1
