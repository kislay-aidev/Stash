# in previous version (ver 32),
# rebuild the logic of writing the stats dictionary for having
# correct update of the category count, and the Execution time.

# But because of this, ended up getting:
# NameError: name 'stats' is not defined

# since, the stats dictionary is created when the loop finishes (after the safe_move function)
# and in "safe_move()" function, have these lines:
# stats["Files Moved"] += 1
# stats["Files Processed"] += 1
# stats["Duplicat Files Renamed"] += 1
# stats["Unknow Files Found"] += 1

# So, switched back to the previous way of writing the stats dict,
# but with a newer approach to cater the "category count", and the "execution time" parameter error:

# --- initialize stats dictionary early ---
# stats = {
#     "Run Time": None,  # will be updated later
#     "Execution Time (seconds)": None,  # will be updated later
#     "Files Processed": 0,
#     "Files Moved": 0,
...
# Finally, after the loop finishes, update the runtime and execution time:

# stats["Run Time"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
# stats["Execution Time (seconds)"] = round(end_time - start_time, 2)
# stats["Category Counts"] = category_counts

# This way, all the errors related to stats dict and its parameters are solved.

# Also changed the way, the movement is logged inside the log file.
# Currently log says:

# Moved: DemoProject.py from FileOrgProject to Python
...
# It is only showing the final folder name.
# So changed it to this format
# Example log:

# [MOVED] Welcome Class.pdf
# FileOrgProject -> Documents/PDFs

# For doing this, only changed this line inside the "Safe_move" function:
# destination_folder = os.path.basename(
    #     os.path.dirname(relative_destination)
    # )

# Here instead of having "destination_folder = os.path.basename(...)"
# changed it to:
# destination_folder = os.path.dirname(relative_destination)

# Note* : NO CHANGE to "relative_destination" function, or other function is made,
# other than the change in "destination_folder" function.
...
# adding "Run Times" to "stats"
stats["Run Time"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
# adding "Execution Time" to "stats"
stats["Execution Time (seconds)"] = round(end_time - start_time, 2)
