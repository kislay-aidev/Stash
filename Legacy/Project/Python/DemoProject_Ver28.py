# In previous versons, if a move fails (permissions, locked file, path too long, etc.), script will crash or silently skip.

# Change - wrapping shutil.move(...) calls in a try/except block with error logging
# With error logging, here capture the failure in the '.log' file alongside successful moves

# BENEFITS:
# No crash: If a file is locked, missing, or permissions fail, your program continues.

# Professional logs: You’ll see both successes and failures in .system/logs/...log.

# Accurate stats: Counters only increment on successful moves/renames. Failures don’t inflate numbers.
